...
failure.
Five of seven DU pairs fall here (TC-CUR-03, 07, 10, 11, 12).
...
The LKW verdict is \textsc{Path\_Infeasible}, confirmed across four
independent executions (one B2 + three B3 at \texttt{temp}$=1.0$).
...
\begin{table*}[t]
\footnotesize
\caption{Stage~1 LKW DU-Pair Verification --- CurrencyAgent.
  All observed values are execution-measured, not code-derived.
  \textbf{B1}: three no-fault runs (\texttt{FAULT\_MODE=NONE},
    \texttt{b2\_raw\_runs/currencyagent\_b2\_run\{1,2,3\}.json}).
  \textbf{B2}: direct \texttt{agent.run()}, no LLM, gRPC mocked to
    $\{\texttt{units{:}9},\allowbreak\texttt{nanos{:}230000000}\}$,
    Concordia Speed HPC Job~1219224
    (\texttt{currencyagent/test\_fault\_injection.py}).
  \textbf{B3}: three live-LLM runs per fault
    (\texttt{temp}$=1.0$,
    \texttt{b3/raw/b3\_\textless FAULT\textgreater\_3b\_temp1.0\_run\{1,2,3\}.json}).}
\label{tab:lkw-stage1}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}p{1.3cm}p{1.9cm}p{1.1cm}p{2.2cm}p{2.5cm}p{3.2cm}p{1.6cm}p{2.3cm}p{2.7cm}p{1.4cm}@{}}
\toprule
\textbf{TC-ID} &
\textbf{Variable} &
\textbf{Fault} &
...
\textbf{B1 observed} &
\textbf{B2 true (no LLM)} &
\textbf{B3 (\texttt{temp}=1.0, 3\,runs)} &
\textbf{Group} \\
\midrule
TC-CUR-01 &
  \texttt{units} & --- &
...
TC-CUR-03 &
  ...
  Group~1 \\[4pt]
TC-CUR-07 &
  \texttt{to\_currency} & FM-1.2 &
  \texttt{agent.py:58} (after swap) &
  \texttt{agent.py:75} \emph{c-use} (gRPC arg) &
  L58$\to$L61$\to$L70$\to$L75 &
  EUR, EUR, EUR &
  \textbf{JPY} \emph{(FAIL)} &
  CAD, CAD, CAD \emph{(FAIL)} &
  Group~1 \\[4pt]
TC-CUR-09 &
  \texttt{data[``units'']} & FM-3.1 &
  \texttt{agent.py:71} (gRPC return) &
  \texttt{agent.py:128} \emph{c-use} (CONVERT\_DONE) &
  L71$\to$\emph{early return at L47--50 kills path}$\to$L128 unreachable &
  9, 9, 9 &
  \textsc{Path\_Infeasible} &
  \textsc{Path\_Infeasible} (3/3) &
  \textbf{Group~2} \\[4pt]
TC-CUR-10 &
  \texttt{data[``units'']} & FM-2.2 &
  \texttt{agent.py:79} (hallucinate) &
  \texttt{agent.py:128} \emph{c-use} (CONVERT\_DONE) &
  L79$\to$L81$\to$L85$\to$L128 &
  9, 9, 9 &
  \textbf{1337} \emph{(FAIL)} &
  1337, 1337, 1337 \emph{(FAIL)} &
  Group~1 \\[4pt]
TC-CUR-11 &
  \texttt{data[``units'']} & BL-RATE &
  ...
  Group~1 \\[4pt]
TC-CUR-12 &
  \texttt{currency\_swapped} & FM-1.2 &
  \texttt{agent.py:134} (boolean expr) &
  \texttt{fi.py:74} \emph{p-use} (rip\_summary) &
  L134$\to$fi.py:74 &
  F, F, F &
  \textbf{True} \emph{(FAIL)} &
  T, T, T \emph{(FAIL)} &
  Group~1 \\
\bottomrule
\end{tabular}}
\smallskip\par\noindent
\textbf{Group~1}: Fault mutates variable at Def; def-clear path to Use still
executes; wrong value arrives at use point; LKW comparison (observed $\neq$
B1 oracle) flags \textsc{Fail}.
\textbf{Group~2} (TC-CUR-09 only): FM-3.1 early return at \texttt{agent.py:47--50}
fires before \texttt{client.convert()} at line~71; both the definition point and
use point are unreachable at runtime; there is no wrong value to observe --- the
sole observable signal is the absence of \texttt{CONVERT\_DONE} from the required
step set $\{\texttt{TASK\_START},\texttt{CONVERT\_DONE},\texttt{FINAL\_ANSWER}\}$.
Confirmed across 4 executions (1 B2 + 3 B3).
\smallskip\par\noindent
$^{\dagger}$\,\texttt{units\_in} absent from B3 oracle comparison framework;
FM-2.5 confirmed via \texttt{amount\_tampered=True} at payment layer in all three
B3 runs. B2 direct measurement (\texttt{units\_in}=50) fills this gap.
\end{table*}
