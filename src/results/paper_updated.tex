...
\subsection{Checkpoint Placement Taxonomy}
\label{sec:placement-taxonomy}

We define five placement classes (P1--P5) that together implement
\textbf{All-Uses} coverage~\cite{rapps-weyuker} for each guarded agent state
variable: P1 records the variable at its primary \emph{definition} (def) site
(task input); P2 and P3 record it at \emph{computation-use} (c-use) sites
(post-computation result and post-side-effect outcome); P4 records
\emph{predicate-use} (p-use) sites where boolean flag values determine
downstream control-flow decisions; P5 records the terminal c-use at the
agent handoff boundary.
Table~\ref{tab:placement-taxonomy} describes each class.

\begin{table}[htb]
\centering
\caption{LKW Checkpoint Placement Classes.}
\label{tab:placement-taxonomy}
\footnotesize
\setlength{\tabcolsep}{4pt}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{clp{3.4cm}p{2.8cm}}
\toprule
\textbf{Class} & \textbf{Placement} & \textbf{Event Captured}
               & \textbf{RIP Purpose} \\
\midrule
P1 & Pre-call         & Incoming payload before any transformation
   & Reachability; input baseline \\
P2 & Post-computation & Core business logic result (e.g., computed cost, fetched records)
   & Infection detection (data corruption) \\
P3 & Post-side-effect & Outcome of irreversible operation (persistence, payment, send)
   & Propagation depth for incomplete-workflow faults \\
P4 & Post-metadata    & Policy or flag field appended after primary logic
   & Tier-2 flag detection (business-logic faults) \\
P5 & Terminal         & Value returned to caller (agent's final answer)
   & Cross-boundary comparison; inter-agent handoff validation \\
\bottomrule
\end{tabular}}
\end{table}

\subsection{Per-Agent Checkpoint Design}

Table~\ref{tab:per-agent-ckpts} lists the instrumented checkpoint sequence for
each of the seven agent services, the data payload guarded at each point, and
the placement class.

\begin{table*}[t]
\centering
\caption{Per-Agent LKW Checkpoint Sequences with Placement Classes and Guarded Data.}
\label{tab:per-agent-ckpts}
\footnotesize
\setlength{\tabcolsep}{3pt}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llp{4.5cm}l}
\toprule
\textbf{Agent} & \textbf{Checkpoint} & \textbf{Guarded Data} & \textbf{Class} \\
\midrule
\multirow{5}{*}{PaymentAgent}
  & TASK\_START     & card token, amount, currency\_code & P1 \\
  & CARD\_VALIDATED & validated=true/false, validation\_bypassed flag & P2 \\
  & CHARGE\_DONE    & charged\_amount, amount\_tampered flag & P3 \\
  & SAVE\_DONE      & save\_skipped flag, record id & P3 \\
  & FINAL\_ANSWER   & result dict returned to caller & P5 \\
\midrule
\multirow{3}{*}{CurrencyAgent}
  & TASK\_START     & from/to currency, amount & P1 \\
  & CONVERT\_DONE   & converted\_amount, hallucinated flag & P2 \\
  & FINAL\_ANSWER   & conversion result dict & P5 \\
\midrule
\multirow{3}{*}{ProductCatalogAgent}
  & TASK\_START     & query, requested product IDs & P1 \\
  & CATALOG\_DONE   & returned product list, phantom\_ids flag & P2 \\
  & FINAL\_ANSWER   & product list returned to caller & P5 \\
\midrule
\multirow{4}{*}{AdServiceAgent}
  & TASK\_START         & context payload & P1 \\
  & CONTEXT\_EXTRACTED  & extracted context features & P2 \\
  & ADS\_FETCHED        & ad list, injected flag & P2 \\
  & FINAL\_ANSWER       & ads returned to caller & P5 \\
\midrule
\multirow{4}{*}{EmailServiceAgent}
  & TASK\_START       & recipient, order data & P1 \\
  & EMAIL\_GENERATED  & body, subject, corrupted flag & P2 \\
  & EMAIL\_SENT       & microservice\_status, boundary\_blocked flag & P3 \\
  & FINAL\_ANSWER     & send result dict & P5 \\
\midrule
\multirow{3}{*}{RecommendationAgent}
  & TASK\_START      & product ID set & P1 \\
  & RECOMMEND\_DONE  & recommended IDs, stale flag & P2 \\
  & FINAL\_ANSWER    & recommendation list & P5 \\
\midrule
\multirow{7}{*}{ShippingService}
  & TASK\_START        & destination address, item count & P1 \\
  & QUOTE\_DONE        & cost\_usd, cost\_corrupted flag & P2 \\
  & CARRIER\_DONE      & carrier, service\_level, ignored\_quote flag & P2 \\
  & TRACKING\_DONE     & tracking\_id & P2 \\
  & SAVE\_DONE         & shipment record, save\_skipped flag & P3 \\
  & ESCALATION\_CHECK  & escalation\_required flag & P4 \\
  & FINAL\_ANSWER      & complete shipment result & P5 \\
\bottomrule
\end{tabular}}
\end{table*}
...
\end{tabular}}
\end{table*}
  & 6/6 & \texttt{boundary\_blocked=True}; send client never called \\
\midrule
\textbf{Total} & & \textbf{19/19} & \\
\bottomrule
\end{tabular}}
