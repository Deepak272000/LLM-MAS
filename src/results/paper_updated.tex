...
workflows from Google Online Boutique (GB). Across the two
complete eight-fault B3 configurations, the checkpoints exposed 41 of 46
conclusive faulty runs (89.1\%, Wilson 95\% CI [77.0, 95.3]\%); two runs were
...
We evaluate the approach on GB e-commerce checkout workflows.
...
\begin{itemize}
  ...
  \item We evaluate eight fault types across GB workflows and report
  ...
  \item We use the traces to analyze first infection, propagation, HITL needs,
        boundary recovery, and the current limits of attribution.
\end{itemize}
...
We study the Google Online Boutique (GB) checkout workflow, an external
cloud-native microservice benchmark. Each agentified service surface has an
...
it is not a claim that all agent surfaces share one deployed call graph. The agent boxes
...
\end{tikzpicture}}
\caption{Common experimental architecture for the GB workflows. Agent
...
figure abstracts the benchmark rather than depicting one deployed call
graph.}
\Description{Conceptual architecture connecting task input to grouped GB agents, controlled service adapters, service interfaces, and a shared Ollama backend. LKW checkpoints observe each agent-to-adapter boundary.}
...
workflow runner; ``B3-8'' denotes participation in the complete eight-fault
...
\toprule
\textbf{Service/agent} & \textbf{Domain} & \textbf{Isolated} & \textbf{Systematic} & \textbf{B3-8} & \textbf{HITL} & \textbf{Measured role} \\
\midrule
...
is one GB workflow instance. An \emph{agentified service} is a service
...
\midrule
Agent & GB role that owns the enclosing trace \\
...
alignment. One row per (agent, fault mode) pair. All agents are from Google
Boutique.
...
BL-* Business fault
      ...
      & GB checkpoints expose different guarded variables per agent
      & Domain labels matter because business harm differs across service roles \\
\bottomrule
\end{tabularx}
\end{table*}
...
RQ5 Boundary recovery potential
...
\begin{table}[t]
\centering
\caption{Complete targeted eight-fault B3 matrix by model configuration. Each
...
\midrule
3b, $t{=}0.7$ & 12 & 10 & 2 & 0 & 22/24 = 91.7\% [74.2, 97.7]\% \\
3b, $t{=}1.0$ & 11 & 8 & 3 & 2 & 19/22 = 86.4\% [66.7, 95.3]\% \\
\midrule
\textbf{Total} & \textbf{23} & \textbf{18} & \textbf{5} & \textbf{2} & \textbf{41/46 = 89.1\% [77.0, 95.3]\%} \\
\bottomrule
\end{tabularx}
\end{table}

The repository also contains lower-temperature summaries, but those campaigns
do not contain all eight current targeted faults and include earlier global
...
complete matrix covers product, currency, payment, and email;
recommendation, ad, and cart are not pooled into its estimate.
Among 46 conclusive runs, 41 exposed at least one deviation (89.1\%, Wilson
95\% CI [77.0, 95.3]\%); two additional runs were inconclusive. The observed
rate was 91.7\% at temperature 0.7 and 86.4\% at temperature 1.0. These counts
show repeatable campaign execution and measured detection.
...
\subsection{RQ5: Can boundary recovery reduce HITL burden?}
...
\subsection{RQ6: Can failure attribution identify the responsible agent?}
...
predictions to compare with this ground truth, so RQ6 remains unanswered as an
...
The third lesson is that business faults should not be merged casually across
service roles. GB faults concern e-commerce data such as prices, products,
payments, and emails, and each owning agent exposes a different guarded
variable. Keeping these roles separate lets us observe where detection
degrades instead of hiding it.
...
workflow studied here. Other domains may require different LKW
checkpoint variables.

\textbf{Unequal campaign coverage.} Only the temperature-0.7 and
temperature-1.0 configurations contain the complete eight-fault targeted matrix.
...
\textbf{Attribution scope.} AgentTracer conversion is complete
