...
\caption{Stage~1 LKW DU-Pair Verification --- CurrencyAgent (Table~A).
  All values are execution-measured, not code-derived.
  ...
  \textbf{B2}: direct \texttt{agent.run()}, no LLM, gRPC mocked to
    $\{\texttt{units{:}9},\texttt{nanos{:}230000000}\}$,
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}p{1.15cm}p{1.85cm}p{2.05cm}p{2.4cm}p{2.85cm}p{2.0cm}p{1.75cm}p{1.5cm}p{1.95cm}p{2.4cm}p{2.1cm}p{1.2cm}@{}}
\toprule
\textbf{TC-ID} &
\textbf{Variable} &
\textbf{Def (file:line)} &
...
\textbf{Expected (B1)} &
\textbf{B1 observed} &
\textbf{B2 observed (no LLM)} &
\textbf{B3 observed (\texttt{temp}=1.0)} &
\textbf{LKW verdict} &
\textbf{Group} \\
\midrule
TC-CUR-01 &
...
\end{tabular}}
\smallskip\par\noindent
\textbf{Group~1}: Fault mutates variable at Def; def-clear path to Use executes; wrong
value arrives at use point; observed~$\neq$~B1 oracle $\Rightarrow$ \textsc{Fail}.
\textbf{Group~2} (TC-CUR-09 only): FM-3.1 inserts an early return at
\texttt{agent.py:47--50}, before \texttt{client.convert()} at line~71.
Both the definition point and the use point are unreachable at runtime---no wrong
value to observe. Sole signal: \texttt{CONVERT\_DONE} absent from required step set
$\{\texttt{TASK\_START},\texttt{CONVERT\_DONE},\texttt{FINAL\_ANSWER}\}$.
Confirmed in 4 independent executions (1 B2 + 3 B3).
\smallskip\par\noindent
$^{\dagger}$\texttt{units\_in} absent from B3 oracle comparison framework;
FM-2.5 confirmed via \texttt{amount\_tampered=True} in all 3 B3 runs.
B2 direct measurement (\texttt{units\_in}=50) fills this oracle gap.
\end{table*}
