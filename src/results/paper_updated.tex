...
deterministic baselines for B3 fault injection (\texttt{USE\_LLM=false});
all seven agents are exercised with live LLM for B2 natural variance collection.
The ShippingService is the sole live-LLM service for fault injection, exercised
with \texttt{qwen2.5-coder:14b} via Ollama on Concordia SPEED HPC. Across all
64~mode-runs ($6\,{\times}\,9$ deterministic~$+$~$10$ ShippingService; each
deterministic mode repeated $3{\times}$ for stability), zero UNSTABLE entries were observed, confirming 100\%
reproducibility of the LKW+RIP instrumentation framework. For the
ShippingService, 9 of 11 fault modes produced positive detectable signals (7~TP,
2~Partial TP); 1 mode was correctly rejected (TN); 1 fault mode remains INCONCLUSIVE due to LLM inference
...
deterministic instrumentation baselines for B3 fault injection, and the
ShippingService agent, which is the sole live-LLM service for fault injection
using \texttt{qwen2.5-coder:14b} via Ollama on Concordia SPEED HPC. B2 natural
variance collection exercises all seven agents with live LLM. This design
separates deterministic framework validation (six agents) from LLM fault-injection
capability testing (ShippingService).
...
orchestrate each microservice via ReAct (dashed; ShippingAgent uses LangGraph;
remaining agents use direct Ollama HTTP) and share a single Ollama LLM backend
(orange, dotted).}
...
Figure~\ref{fig:rip-causality} shows how the FM-2.2 mutation operator maps
to a specific infection checkpoint at each agent.  All seven agents share the
same P2 c-use placement class and Tier~3 (Silent) classification, confirming
that the LKW framework produces consistent, evidence-based tier assignment
across the full service portfolio.

\begin{figure*}[!tp]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig_rip_causality.pdf}
\caption{RIP Causality for FM-2.2 (Hallucinated Output): primary infection
  checkpoint, LKW criterion, use type, and HITL tier for each agent.  All
  seven agents produce a P2 c-use infection with Tier~3 Silent classification,
  confirming structural invariance of the FM-2.2 trace signature across the
  service portfolio.}
\label{fig:rip-causality}
\end{figure*}
...
FM-2.5 & Input suppression & upstream numeric value
  & P2 c-use (e.g.\ \texttt{CARRIER\_DONE}) & $\delta_{\epsilon}$ & 2 \\
...
\begin{description}
  ...
  \item[\textbf{B3 --- Fault injection}] The same agent with
        ...
        collected for all seven agents across three \texttt{qwen2.5:3b}
        temperature configurations (30 runs total).
\end{description}

At temperature~=~0, \texttt{ship\_order} is never reached; at 0.7 and~1.0,
partial reach is observed, reflecting natural LLM routing variance.
...
\begin{itemize}
  \item \textbf{Six deterministic agents} (Payment, Currency, Email,
        ...
        LKW+RIP instrumentation framework on deterministic Python code during
        B3 fault injection. For B2 natural variance collection, all seven agents
        are exercised with live LLM (Section~\ref{sec:b1-b2-align}).
  \item \textbf{ShippingService}: the sole live-LLM agent for B3 fault injection.
        The full LangGraph
...
\begin{figure*}[!tp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_hitl_stacked.pdf}
\caption{HITL Tier Distribution per Agent.  Tier~1 (red, structural) faults
  are detected automatically via step-count comparison; Tier~2 (orange,
  flag-detectable) require a flag monitor on LKW checkpoint data; Tier~3
  (grey, silent) require semantic payload validation against the B1 oracle.
  ShippingService (Shipping) contributes all three tiers due to its live-LLM
  execution path.}
...
BL     & Business Logic        & Middle checkpoint      & 0    & Tier~1--2           \\
\midrule
---    & NONE (baseline)       & ---                    & 0    & ---                 \\
\bottomrule
...
\node[draw,fill=yellow!12,rounded corners=5pt,inner sep=8pt,thick,
      font=\small,align=center,minimum width=16cm]
  at (3.45, -3.35)
  {\textbf{Key Finding: FM-2.2 (Tier~3 Silent) crosses service boundaries with zero
   structural signal --- no LKW step loss at either hop.}\\[4pt]
   Chain~A: \textbf{14{,}755\% financial overcharge blocked by
   \texttt{block\_and\_request\_hitl} recovery policy}
   (\texttt{charge\_blocked\,=\,True}).\\
   Chain~B: Semantic fault self-contained ---
   downstream returns baseline-identical recommendations (no financial risk).};

\end{tikzpicture}%
}
\caption{Cross-agent FM-2.2 (Hallucinated Output, Tier~3 Silent) propagation.
  \textit{Red}: upstream agents with FM-2.2 injected.
  \textit{Green}: fault-free downstream agents (all LKW checkpoints reached at both hops).
  \textit{Gray}: boundary monitors emit \texttt{BOUNDARY\_CHECK} alerts.
  Chain~A (financial): CurrencyAgent hallucinates 1{,}337\,EUR (baseline 9\,EUR; +14{,}755\%);
  \texttt{block\_and\_request\_hitl} fires at the boundary --- charge never executed.
  Chain~B (semantic): ProductCatalogAgent returns empty product list [\,];
  RecommendationAgent returns baseline-identical recommendations (no financial exposure).
  \emph{Neither chain produces a structural LKW step-loss signal}: both faults are
  Tier~3 Silent and detectable only by semantic payload inspection.}
...
B: Catalog$\to$Recommendation
  & \texttt{catalog\_to\_recommendation} & \texttt{PROD-001} & \texttt{[]\ (empty)}
  & Yes (missing/extra) \\
\bottomrule
\end{tabular}}
\end{table*}
...
Only \textbf{ShippingService} runs live LLM during B3 fault injection campaigns.
...
Pure LLM ReAct orchestrators have been built for all six agents and used for B2
natural variance collection; extending B3 fault
injection to live-LLM execution for the remaining six agents is left as future work.
...
For the ShippingService Retail-bench, 9 of 11 fault modes produced positive
detectable signals (7~TP, 2~Partial TP); 1 mode was correctly rejected (TN); 1 fault mode
