...
an external cloud-native microservice benchmark, whereas RB is a
single-orchestrator shipping workflow constructed for this study that exposes
quote and order phases as two entry points. Each surface has an
...
\subsection{RetailBen Shipping Orchestrator (RB)}

\begin{sloppypar}
RetailBen (RB) is a shipping workflow constructed for this study. It is
implemented as a \emph{single} orchestrator module: the
\texttt{Shipping\-Orchestrator} class, defined in
\texttt{orchestrator.py} under \texttt{shippingagent/app/}.
This module drives a hand-written
ReAct loop (Reason--Act--Observe) against a self-hosted Llama-family
OpenAI-compatible endpoint and dispatches tool calls to three internal helpers
(\texttt{QuoteAgent}, \texttt{CarrierSelectionAgent}, \texttt{TrackingAgent}).
The orchestrator exposes two asynchronous entry points, which we treat as two
logical phases of one workflow rather than as two separate agents:
\end{sloppypar}

\begin{description}
  \item[Quote phase (\texttt{get\_quote}).] Receives package, destination, and
        inventory context and returns a price quote. Its checkpoints expose
        whether inventory or quote information changes before the result is
        passed downstream.
  \item[Order phase (\texttt{ship\_order}).] Re-derives the quote, then selects
        a carrier, creates a tracking record, saves the order, and evaluates
        escalation conditions. Because it consumes the upstream quote, this
        phase reveals whether an error is blocked, corrected, or propagated
        into order state.
\end{description}

Both entry points instantiate the same \texttt{LKWCheckpoint} class, whose
single \texttt{EXPECTED\_STEPS} sequence (\texttt{TASK\_START},
\texttt{QUOTE\_DONE}, \texttt{CARRIER\_DONE}, \texttt{TRACKING\_DONE},
\texttt{SAVE\_DONE}, \texttt{FINAL\_ANSWER}) spans the whole workflow. RB
therefore has one Stage~1 verification table
(Table~\ref{tab:lkw-stage1-shipping}) covering both phases, rather than one per
phase. Where the two phases must be distinguished --- as separate
\texttt{infection\_agent} values in the B3 traces, and in the coverage and
fault-map tables --- we retain the labels \textbf{ShippingQuoteAgent} and
\textbf{ShipOrderAgent} for the quote and order phases respectively. These are
phase labels for one module, not names of separate modules.
...
\begin{table*}[t]
\centering
\caption{Fault-to-checkpoint instrumentation map with service-interface
alignment. One row per (agent, fault mode) pair. Agents marked \textbf{(RB)}
are from RetailBen; all other agents are from Google Boutique.
\textbf{Agent checkpoint} is the first LKW point where the injected fault is
expected to become observable. \textbf{Microservice observation point} is the
service-interface surface against which that checkpoint is compared.
\textbf{AU} = All-Uses; \textbf{ADP} = All-DU-Paths. \textbf{C-use} means
computational use and \textbf{P-use} means predicate/decision use.
Comparison-relation classes are stated per checkpoint in the paragraph below
rather than repeated per row. This is a design table, not a results table.}
\label{tab:checkpoint-fault-merged}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}p{2.45cm}p{2.55cm}p{4.35cm}p{2.75cm}ll@{}}
\toprule
\textbf{Agent} & \textbf{Agent checkpoint} & \textbf{Microservice observation point} & \textbf{Fault mode} & \textbf{Criterion} & \textbf{Use} \\
\midrule
ShippingQuoteAgent \textbf{(RB)} & \texttt{TASK\_START} & Shipping quote service request/response values (inventory status, carrier, quote, ETA) & \texttt{BL-INV-MISMATCH} & \textbf{AU/ADP} & C-use/P-use \\
 & \texttt{QUOTE\_DONE} & & \texttt{BL-REFUND} & \textbf{ADP} & C-use/P-use \\
 & \texttt{QUOTE\_DONE} & & \texttt{FM-2.5} & \textbf{ADP} & C-use/P-use \\
\midrule
ShipOrderAgent \textbf{(RB)} & \texttt{ESCALATION\_CHECK} & Ship-order service request/response values (carrier, tracking ID, save status, escalation) & \texttt{BL-ESCALATION} & \textbf{AU} & P-use \\
 & \texttt{SAVE\_DONE} & & \texttt{BL-SHIP-LOST} & \textbf{ADP} & C-use/P-use \\
 & \texttt{CARRIER\_DONE} & & \texttt{BL-VENDOR} & \textbf{AU} & C-use \\
 & \texttt{TRACKING\_DONE} & & \texttt{FM-1.2} & \textbf{AU} & C-use \\
 & \texttt{CARRIER\_DONE} & & \texttt{FM-2.2} & \textbf{AU} & C-use \\
 & \texttt{CARRIER\_DONE} & & \texttt{FM-2.5} & \textbf{AU} & P-use \\
 & \texttt{TRACKING\_DONE} & & \texttt{FM-3.1} & \textbf{AU} & C-use \\
\midrule
PaymentAgent & \texttt{CARD\_VALIDATED} & PaymentService request/response values (charged amount, transaction outcome) & \texttt{BL-AMOUNT-TAMPER} & \textbf{AU/ADP} & C-use/P-use \\
 & \texttt{SAVE\_DONE} & & \texttt{BL-DOUBLE-CHARGE} & \textbf{AU} & P-use \\
 & \texttt{SAVE\_DONE} & & \texttt{BL-TXN-LOST} & \textbf{AU} & P-use \\
 & \texttt{CARD\_VALIDATED} & & \texttt{FM-1.2} & \textbf{AU} & P-use \\
 & \texttt{CHARGE\_DONE} & & \texttt{FM-2.2} & \textbf{AU} & C-use/P-use \\
 & \texttt{CARD\_VALIDATED} & & \texttt{FM-2.5} & \textbf{AU/ADP} & C-use/P-use \\
\midrule
CurrencyAgent & \texttt{CONVERT\_DONE} & CurrencyService request/response values (source, target, converted amount) & \texttt{BL-CONV-OVERFLOW} & \textbf{AU} & C-use \\
 & \texttt{CONVERT\_DONE} & & \texttt{BL-RATE-MANIP} & \textbf{AU} & C-use \\
 & \texttt{CONVERT\_DONE} & & \texttt{BL-STALE-RATE} & \textbf{AU} & C-use \\
 & \texttt{CONVERT\_DONE} & & \texttt{FM-1.2} & \textbf{AU} & C-use/P-use \\
 & \texttt{CONVERT\_DONE} & & \texttt{FM-2.2} & \textbf{AU} & C-use \\
 & \texttt{CONVERT\_DONE} & & \texttt{FM-2.5} & \textbf{AU} & C-use \\
\midrule
EmailServiceAgent & \texttt{EMAIL\_GENERATED} & EmailService request/response values (recipient, content fields, send status) & \texttt{BL-CORRUPT-BODY} & \textbf{AU} & C-use \\
 & \texttt{EMAIL\_SENT} & & \texttt{BL-SEND-SKIP} & \textbf{AU} & C-use/P-use \\
 & \texttt{EMAIL\_GENERATED} & & \texttt{FM-1.2} & \textbf{AU} & C-use/P-use \\
 & \texttt{EMAIL\_GENERATED} & & \texttt{FM-2.2} & \textbf{AU} & P-use \\
 & \texttt{EMAIL\_GENERATED} & & \texttt{FM-2.5} & \textbf{AU} & C-use \\
\midrule
ProductCatalogAgent & \texttt{CATALOG\_DONE} & ProductCatalogService response values (product IDs, prices, list contents) & \texttt{BL-DUP-PRODUCT} & \textbf{AU} & C-use/P-use \\
 & \texttt{CATALOG\_DONE} & & \texttt{BL-PRICE-MANIP} & \textbf{ADP} & C-use \\
 & \texttt{CATALOG\_DONE} & & \texttt{BL-PROD-MISSING} & \textbf{AU} & C-use/P-use \\
 & \texttt{CATALOG\_DONE} & & \texttt{FM-1.2} & \textbf{AU} & P-use \\
 & \texttt{CATALOG\_DONE} & & \texttt{FM-2.2} & \textbf{AU} & C-use/P-use \\
\midrule
RecommendationAgent & \texttt{RECOMMEND\_DONE} & RecommendationService request/response values (context IDs, recommendation IDs) & \texttt{BL-EMPTY-RECS} & \textbf{AU} & C-use/P-use \\
 & \texttt{RECOMMEND\_DONE} & & \texttt{BL-INJECT-RECS} & \textbf{AU} & P-use \\
 & \texttt{RECOMMEND\_DONE} & & \texttt{FM-2.2} & \textbf{AU} & C-use \\
 & \texttt{RECOMMEND\_DONE} & & \texttt{FM-2.5} & \textbf{AU} & C-use \\
 & \texttt{RECOMMEND\_DONE} & & \texttt{FM-3.1} & \textbf{AU} & C-use/P-use \\
\midrule
AdServiceAgent & \texttt{ADS\_FETCHED} & AdService request/response values (context fields, ad IDs/URLs) & \texttt{BL-AD-INJECTION} & \textbf{AU} & C-use/P-use \\
 & \texttt{ADS\_FETCHED} & & \texttt{BL-DUP-ADS} & \textbf{AU} & C-use/P-use \\
 & \texttt{ADS\_FETCHED} & & \texttt{BL-EMPTY-ADS} & \textbf{AU} & C-use/P-use \\
 & \texttt{CONTEXT\_EXTRACTED} & & \texttt{FM-1.2} & \textbf{AU} & C-use \\
 & \texttt{ADS\_FETCHED} & & \texttt{FM-2.2} & \textbf{AU} & C-use \\
 & \texttt{CONTEXT\_EXTRACTED} & & \texttt{FM-2.5} & \textbf{AU} & C-use \\
\bottomrule
\end{tabular}
\end{table*}

\smallskip\par\noindent
\textbf{Comparison-relation classes, by checkpoint.}
Each checkpoint is compared to its service-interface observation point under a
fixed relation class, declared once here rather than repeated per row.
$\delta_{\epsilon}$ denotes a numeric exact, tolerance, or range check;
$\mathcal{S}$ denotes string, set, schema, or required-step conformance; and
$\Sigma$ denotes boolean or diagnostic-flag conformance.
\texttt{CONVERT\_DONE} (CurrencyAgent) uses $\delta_{\epsilon}$.
\texttt{CHARGE\_DONE} (PaymentAgent) uses $\delta_{\epsilon}$ on the charged
amount plus $\Sigma$.
\texttt{CATALOG\_DONE} (ProductCatalogAgent) uses $\mathcal{S}$ plus
$\delta_{\epsilon}$.
\texttt{EMAIL\_GENERATED}/\texttt{EMAIL\_SENT} (EmailServiceAgent) use
$\mathcal{S}$ plus $\Sigma$.
\texttt{RECOMMEND\_DONE} (RecommendationAgent) and \texttt{ADS\_FETCHED}
(AdServiceAgent) use $\mathcal{S}$.
\texttt{CART\_DONE} (CartAgent) uses $\mathcal{S}$ plus $\delta_{\epsilon}$;
CartAgent has isolated harness evidence and is therefore aligned to its service
interface but carries no rows in the fault map above.
\texttt{QUOTE\_DONE} (ShippingQuoteAgent, RB) uses $\delta_{\epsilon}$ plus
$\mathcal{S}$.
\texttt{CARRIER\_DONE}/\texttt{TRACKING\_DONE}/\texttt{SAVE\_DONE}
(ShipOrderAgent, RB) use $\mathcal{S}$ plus $\Sigma$.
B1 supplies expected values and B2 characterizes acceptable no-fault variation
for all of these comparisons.

The checkpoint-equivalence relations in
Table~\ref{tab:checkpoint-fault-merged} are declared before the B3 campaign
using B1 semantics and B2 no-fault observations. B3 is then evaluated under
these fixed relations so calibration and evaluation remain separated.

Table~\ref{tab:checkpoint-fault-merged} also serves as the field-level
instrumentation map for the Python service-facing agents. It is a design table,
not a results table. The checkout orchestrator is evaluated structurally
through expected-step checks, and CartAgent has isolated harness evidence but
carries no rows in the field-level map. The table shows where each listed fault
should become visible, the service-interface surface it is compared against,
and the LKW criterion and use type.
...
In measured cross-agent Chain A (Table~\ref{tab:payment-example}), the expected
amount at the currency-to-payment boundary was 9 units. FM-2.2 changed the
observed boundary amount to 1337 units, a difference of 1328 units
(14,755.6\%). The trace also
...
is no Python ReAct orchestrator runtime.
...
use points.
Table~\ref{tab:lkw-stage1-shipping} covers 7 fault-tested DU pairs
(2~Group~2, 5~Group~1) drawn from both general FM faults and five
RetailBen-specific business faults.

\textbf{Group~1 --- wrong value at the use point.}
Five DU pairs (TC-SHP-03, 05, 07, 08, 09) fall here.
...
B3 measured as co-resident agents in the GB checkout campaign, pooled over
\texttt{3b\_temp0.7} and \texttt{3b\_temp1.0} (six runs per fault; see
footnote~[3]).}
...
TC-SHP-02 &
  ...
  \textbf{PATH\_INFEAS.}$^{[3]}$ 3/6 \emph{absent} (B1=DHL); 3/6 not reached &
...
TC-SHP-04 &
  ...
  \textbf{PATH\_INFEAS.}$^{[3]}$ 2/6 \emph{absent} (B1=8434097984); 4/6 not reached &
  PATH\_INFEAS. --- use never reached &
...
TC-SHP-05 &
  ...
  \textbf{FAIL}$^{[3]}$ 4/6 \emph{ign.\ dstream quote}=True (detects FM-2.5) &
  \textbf{FAIL} in B2 &
  \textbf{Group~1} --- LKW catches FM-2.5 \\[4pt]
TC-SHP-07 &
  \texttt{carrier} &
  ...
  \textbf{FAIL}$^{[3]}$ 3/6 \emph{absent} (B1=DHL; all $t{=}1.0$) &
  \textbf{FAIL} in B2 &
...
TC-SHP-08 &
  ...
  \textbf{FAIL}$^{[3]}$ 3/6 \emph{absent} (B1=15.88; all $t{=}1.0$) &
  \textbf{FAIL} in B2 &
...
TC-SHP-09 &
  ...
  \textbf{False}$^{[1]}$ &
  \textbf{True}$^{[2]}$ (FAIL) &
  \textbf{FAIL}$^{[3]}$ 3/6 =True (detects BL-CUST-ESC); 3/6 \emph{absent} &
  \textbf{FAIL} in B2 &
...
\smallskip\par\noindent
\textbf{[3]}~B3 \emph{was} measured for RetailBen, but as \emph{co-resident} agents
inside the Google Boutique checkout campaign rather than as a standalone RB run.
Both RB agents are instrumented and observable in that pipeline: the fault matrix
records \texttt{infection\_agent} values \texttt{shipping\_quote} and
\texttt{ship\_order}, and the two RB business faults carried in the pooled
ten-fault matrix detect at
$5/6 = 83.3\%$ each (BL-INVENTORY\_MISM.\ via \texttt{shipping\_quote};
BL-SHIPMENT\_LOST via \texttt{ship\_order}; see
Table~\ref{tab:b3-fault-summary-simple}).
The per-row B3 cells above are pooled over the six runs
$\{$\texttt{3b\_temp0.7}, \texttt{3b\_temp1.0}$\}\times\{$run1--3$\}$, read from
\texttt{b3/raw/b3\_<FAULT>\_<CONFIG>\_run<N>.json} and aggregated in
\texttt{b3/b3\_<FAULT>\_<CONFIG>\_summary.json}.

\smallskip\par\noindent
\emph{Two caveats we state explicitly rather than smooth over.}
(i)~Most RB B3 deviations are recorded at \texttt{severity="missing"} ---
the checkpoint is \emph{absent} rather than carrying a wrong value --- because
the RB agents sit late in the checkout order and are frequently not reached
under an active fault. Absence is a genuine LKW signal (it is exactly what
Group~2 is defined to catch), but it is weaker evidence than a value mismatch,
and only TC-SHP-05 and TC-SHP-09 produce a populated \texttt{detects} field.
(ii)~TC-SHP-03 (FM-2.2) shows \textbf{no} \texttt{carrier} deviation in any of
the six B3 runs, so its B2 FAIL result does \emph{not} reproduce under the live
LLM; we report it as NOT DETECTED rather than counting it as a B3 success.
This does \emph{not} contradict the $6/6 = 100\%$ pooled rate reported for FM-2.2
in Table~\ref{tab:b3-fault-summary-simple}: that rate is workflow-level and is
driven entirely by upstream deviation at \texttt{productcatalog}, which is the
recorded \texttt{infection\_agent} for every FM-2.2 run.
ShipOrderAgent-specific detection for FM-2.2 is $0/6$.
The two figures measure different scopes --- whether \emph{any} agent deviated
versus whether \emph{this} agent deviated --- and both are reported as measured.

\smallskip\par\noindent
\textbf{[4]}~A separate standalone RB campaign was run at
\texttt{qwen2.5-coder:14b}, $t{=}0$ (the \texttt{retail\_bench} block of
\texttt{b3/b3\_full\_report.json}; ten fault modes $\times$ three runs).
It returned \texttt{detection\_rate}~$=0.0$ for every fault mode.
That campaign is \emph{excluded} from every pooled figure in this paper --- all
headline B3 numbers use \texttt{qwen2.5:3b} --- and we report it here only so the
null result is on the record.
...
relations. Table~\ref{tab:injection-design} states, for each evaluated fault
class, how it was injected, how the LKW checkpoints observe it, and how the
resulting evidence should be interpreted.
...
attribution accuracy. The evidence scope is stated separately for each RQ, and
Table~\ref{tab:rq-evidence-map} maps each research question to its primary
evidence, measurement, and confidence reporting.
...
RQ2 First infection location
      & Tables~\ref{tab:checkpoint-fault-merged}, \ref{tab:b3-fault-summary-simple}
...
checkpoint or handoff. Table~\ref{tab:checkpoint-fault-merged} gives the expected
...
Primary evidence comes from Tables~\ref{tab:checkpoint-fault-merged} and
