...
Every value in the B1, B2, and B3 columns is from an actual execution artifact, not derived by reading the code.
\textbf{B1 source:}
  \texttt{b2\_raw\_runs/currencyagent\_b2\_run\{1,2,3\}.json} ---
  \texttt{FAULT\_MODE=NONE}, real LKW trace, no LLM.
\textbf{B2 source:}
  \texttt{b2\_currency\_true\_b2.json} ---
  \texttt{agent.run()} called directly, no LLM, mocked gRPC
  (\texttt{units}$\to$9 fixed return), each fault mode run once.
  Run: \texttt{currencyagent/test\_fault\_injection.py}.
\textbf{B3 source:}
  \texttt{b3/raw/b3\_<FAULT>\_3b\_temp1.0\_run\{1,2,3\}.json} ---
  non-deterministic LLM (\texttt{temp}$=1.0$) with fault active, 3 runs per fault.}
...
\smallskip\par\noindent
$^{1}$~B1 values measured from \texttt{CONVERT\_DONE} and \texttt{TASK\_START}
checkpoints in \texttt{b2\_raw\_runs/currencyagent\_b2\_run\{1,2,3\}.json}
(3 runs, all identical).

\smallskip\par\noindent
$^{2}$~B2 values measured from \texttt{b2\_currency\_true\_b2.json} ---
...
\smallskip\par\noindent
$^{3}$~B3 values from \texttt{b3/raw/b3\_<FAULT>\_3b\_temp1.0\_run\{1,2,3\}.json}.
TC-CUR-03 B3 shows NOT\_MEASURED because \texttt{units\_in} was not included in the
oracle comparison framework; FM-2.5 firing is confirmed by
\texttt{payment.amount\_tampered=True} in all 3 runs.
TC-CUR-07 B3: LLM chose \texttt{to\_currency=CAD}; FM-1.2 swapped CAD$\to$JPY;
CAD deviation reflects LLM input variance; \texttt{currency\_swapped=True}
confirms the swap fired.
TC-CUR-09 B3: \texttt{units\_out=None} with severity \texttt{missing} in all 3 runs,
both temp0 and temp1.0 --- 6 consecutive executions confirm PATH\_INFEASIBLE.
