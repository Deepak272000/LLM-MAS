...
All 17 tracked fields show zero variance across all 10 runs: B2$=$B1 confirmed
...
currency fields: \texttt{CurrencyAgent.app/agent.py} contains no LLM invocation;
the ReAct orchestrator layer does not modify field values for this agent.
Noise envelope: zero variance on all numeric and Boolean fields.
...
(10 live-LLM NONE runs, \texttt{qwen2.5:3b}, temp$=0.0$).
Live-LLM B2 variance: \texttt{CHARGE\_DONE.transaction\_id} varies per run
(all valid UUID-v4; schema\_regex comparison confirmed).
\texttt{TASK\_START.units} and \texttt{CHARGE\_DONE.units\_charged} show natural
LLM orchestrator variance: 24 in 8/10 runs, 34 in 2/10 runs.
...
\texttt{agent.run()} directly (no LLM, MongoDB mocked).
Live-LLM noise envelope (\texttt{b2\_live\_llm/}): \texttt{units\_charged}$\in\{24,34\}$
across 10 NONE runs; BL-AMT-TAMP inflates to 72 ($3{\times}$ base of 24),
exceeding the live-LLM noise ceiling of 34 --- fault remains detectable.
...
(10 live-LLM NONE runs, \texttt{qwen2.5:3b}, temp$=0.0$).
All 11 tracked fields show zero variance across all 10 runs: B2$=$B1 confirmed
under live-LLM pipeline conditions.
...
\textbf{Live-LLM B2 status:} AdServiceAgent has a \texttt{USE\_LLM} flag
(\texttt{app/graph.py:48}) but was run with \texttt{USE\_LLM=false} for
these traces.
A live-LLM rerun (\texttt{USE\_LLM=true}, \texttt{FAULT\_MODE=NONE}) has not
been executed; live-LLM B2 characterization is pending.
AdServiceAgent is also not in the B3 checkout pipeline.
...
\textbf{Note:} B2 is from the deterministic execution path;
no live-LLM isolated fault injection exists for AdServiceAgent.
...
B1 from \texttt{b2\_systematic/raw/checkout\_3b\_temp1.0\_NONE\_run\{1,2,3\}.json}
(live LLM, FAULT\_MODE=NONE);
...
TC-CO-01 &
  ...
  \textbf{8 steps (7/10 runs)}$^{[1]}$; CONFIRMATION\_SENT absent 3/10 (LLM non-det.) &
  NOT\_MEASURED &
...
3/10 NONE runs omit CONFIRMATION\_SENT (LLM non-determinism at temp=1.0).
