...
\begin{description}
  ...
  \item[\textbf{B3 --- Fault injection}] The same agent with
        ...
        temperature configurations (30 runs total; see Figure~\ref{fig:b2-heatmap}).
\end{description}

Figure~\ref{fig:b2-heatmap} visualises the B2 agent reach rate across the
three \texttt{qwen2.5:3b} temperature configurations, showing how temperature
affects LLM routing through the checkout pipeline.

\begin{figure*}[t]
\centering
\includegraphics[width=0.90\textwidth]{figures/fig_b2_heatmap.pdf}
\caption{B2 Natural Variance Baseline --- per-agent reach rate (fraction of
  10~runs where \texttt{TASK\_START} was recorded) across three temperature
  configurations with no fault injection.  Green~=~1.0, Red~=~0.0.
  At temp~=~0, \texttt{ship\_order} is never reached (LLM consistently
  short-circuits the checkout after \texttt{payment}); at temp~=~0.7 and 1.0,
  partial reach is observed, reflecting natural LLM routing variance.}
\label{fig:b2-heatmap}
\end{figure*}
...
Figure~\ref{fig:lkw-trace-diff} shows a concrete killed-mutant example for
PaymentAgent \texttt{BL-AMOUNT-TAMPER}.

\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{figures/fig_lkw_trace_diff.pdf}
\caption{B1 Oracle vs.\ B3 Fault Run for PaymentAgent \texttt{BL-AMOUNT-TAMPER}.
  The B3 trace deviates at \texttt{CHARGE\_DONE} (P3 class):
  \texttt{charged=9999.00} vs.\ B1 baseline \texttt{9.00}, with
  \texttt{tampered=True} set.  All steps are reached (propagation depth~=~0),
  so this is a Tier~3 Silent fault --- detectable only by comparing the P2/P3
  data payload against the B1 oracle value.}
\label{fig:lkw-trace-diff}
\end{figure*}
...
Figure~\ref{fig:stability-scatter} shows per-fault-mode average detection
rates coloured by HITL tier, confirming that all TP faults achieve 100\%
or near-100\% detection stability.

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig_stability_scatter.pdf}
\caption{Fault Detection Stability: average detection rate per fault mode
  across all agents and configurations.  Points coloured by HITL Tier (Tier~1
  red, Tier~2 orange, Tier~3 grey).  Most Tier~1 and Tier~2 faults achieve
  100\% detection; Tier~3 (Silent) faults cluster at lower detection rates,
  reflecting the need for semantic payload comparison.}
\label{fig:stability-scatter}
\end{figure*}
...
\node[res] (resb) at ( 5.0, -1.2)
  {Default recs\\(baseline pattern)\\\textcolor{blue}{unaffected}};
\draw[->,red!70] (pc) -- node[above,font=\tiny,align=center]
  {empty\\product list} (ra);
\draw[->] (ra) -- node[above,font=\tiny] {baseline recs} (resb);
...
Figure~\ref{fig:cross-agent-chain} traces the data-flow and boundary signals
for Chain~A in detail.

\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{figures/fig_cross_agent_chain.pdf}
\caption{Cross-Agent FM-2.2 Propagation Chain~A (financial): CurrencyAgent
  hallucination (1{,}337\,EUR) propagates through the currency-to-payment
  boundary where the recovery policy fires \texttt{block\_and\_request\_hitl},
  preventing the unsafe charge.  LKW checkpoint labels and boundary
  alert values are shown at each stage.}
\label{fig:cross-agent-chain}
\end{figure*}
...
natural variance collection (Figure~\ref{fig:b2-heatmap}); extending B3 fault
